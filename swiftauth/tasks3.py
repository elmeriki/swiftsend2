...
@shared_task
def send_bulk_template2_emails_task(id,company_username):
    ...
    save_email = Messagescontent(id=unique_badge_id,subject=template_instance.templatename,message=template_instance.abouttext,sent=True,error_message="None",type="Email")
    save_email.save() 
    ...
    for user in users:
        try:
            ...
            save_email_log=EmailLog(messagecontent=email_sent_instance,subject=template_instance.templatename,email=user.email,fname=user.first_name,lname=user.last_name,message=template_instance.abouttext,sent=True,error_message="None")
            save_email_log.save()
        except Exception as e:
            # Handle exceptions
            email_sent_instance=Messagescontent.objects.get(id=unique_badge_id)
            save_email_log=EmailLog(messagecontent=email_sent_instance,subject=template_instance.templatename,email=user.email,fname=user.first_name,lname=user.last_name,message=template_instance.abouttext,sent=False,error_message=str(e))
            save_email_log.save()      
